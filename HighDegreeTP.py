...
class TwoPhaseICM:
    ...
    def select_seeds_with_budget(self, candidate_nodes, budget):
        """High Degree algorithm implementation"""
        selected = set()
        remaining_budget = budget
        
        # Sort nodes by degree (in-degree for directed graphs) in descending order
        if self.is_directed:
            nodes_sorted = sorted(candidate_nodes, 
...
def save_split_ratio_results(results, split_ratio, base_filename="HighDegree_Two_Phase_Results"):
...
def main():
    ...
    print("Running experiments with High Degree algorithm...")
