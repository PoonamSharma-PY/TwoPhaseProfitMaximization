...
class TwoPhaseICM:
    ...
    def _set_activation_probabilities(self):
        if self.model_type == 'uniform':
            ...
            for u, v in self.graph.edges():
                degree = self.graph.in_degree(v) if self.is_directed else self.graph.degree(v)
                self.graph[u][v]['weight'] = 1.0 / degree if degree != 0 else 0.0

    def _precompute_clustering(self):
        """Precompute clustering coefficients for all nodes"""
        self.clustering = {}
        for node in self.nodes:
            if self.is_directed:
                # For directed graphs, use clustering coefficient that considers directed triangles
                self.clustering[node] = nx.clustering(self.graph, nodes=node, weight='weight')
            else:
                self.clustering[node] = nx.clustering(self.graph, nodes=node)
    ...
    def select_seeds_with_budget(self, candidate_nodes, budget):
        """High Clustering Coefficient algorithm implementation"""
        selected = set()
        remaining_budget = budget
        
        # Create list of nodes with their clustering coefficients and costs
        node_data = []
        for node in candidate_nodes:
            node_data.append({
                'node': node,
                'clustering': self.clustering.get(node, 0),
                'cost': self.costs.get(node, 0)
            })
        
        # Sort nodes by clustering coefficient (descending) and cost (ascending)
        node_data.sort(key=lambda x: (-x['clustering'], x['cost']))
        
        for node_info in node_data:
            node = node_info['node']
            node_cost = node_info['cost']
...
def save_split_ratio_results(results, split_ratio, base_filename="HighClustering_Two_Phase_Results"):
        ...
        for budget in tqdm(budgets, desc=f"Budgets (split={split_ratio})"):
            for timestep in tqdm(timesteps, desc="Timesteps", leave=False):
                ...
                result = {
                    ...
                    'Remaining_Budget': budget - final_seed_cost
                }
...
def main():
    ...
    print("Running experiments with High Clustering Coefficient algorithm...")
