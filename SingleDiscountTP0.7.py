...
class TwoPhaseICM:
    ...
    def select_seeds_with_budget(self, candidate_nodes, budget):
        """Single Discount algorithm implementation"""
        selected = set()
        remaining_budget = budget
        node_degrees = defaultdict(int)
        
        # Initialize degrees for all candidate nodes
        for node in candidate_nodes:
            if self.is_directed:
                node_degrees[node] = self.graph.in_degree(node)
            else:
                node_degrees[node] = self.graph.degree(node)
        
        while remaining_budget > 0 and node_degrees:
            # Select node with maximum degree
            max_node = max(node_degrees.items(), key=lambda x: x[1])[0]
            node_cost = self.costs.get(max_node, 0)
            
            if node_cost > remaining_budget:
                del node_degrees[max_node]
                continue
                
            selected.add(max_node)
            remaining_budget -= node_cost
            
            # Discount degrees of neighbors (single discount)
            neighbors = list(self.graph.successors(max_node)) if self.is_directed else list(self.graph.neighbors(max_node))
            for neighbor in neighbors:
                if neighbor in node_degrees:
                    node_degrees[neighbor] -= 1  # Single discount: subtract 1 from degree
...
def save_split_ratio_results(results, split_ratio, base_filename="SingleDiscount_Two_Phase_Results"):
...
def main():
    ...
    print("Running experiments with Single Discount algorithm...")
