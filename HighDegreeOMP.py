...
def high_degree_algorithm(graph, budget, cost_dict):
    if isinstance(graph, nx.DiGraph):
        degree_dict = {node: graph.out_degree(node) for node in graph.nodes()}
    else:
...
def benchmark_high_degree():
    ...
    for model_name, graph_path in GRAPH_VERSIONS.items():
        ...
        for budget in BUDGETS:
            ...
            start = time.time()
            seed_set, seed_cost = high_degree_algorithm(graph, budget, costs)
        ...
        output_file = f"HighDegree_Numba_Results_{model_name}.xlsx"
        pd.DataFrame(results).to_excel(output_file, index=False)
        print(f"✅ Saved to {output_file}")

if __name__ == "__main__":
    benchmark_high_degree()
