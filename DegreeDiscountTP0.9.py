...
class TwoPhaseICM:
    ...
    def select_seeds_with_budget(self, candidate_nodes, budget, p=0.1):
        """Degree Discount algorithm implementation"""
        selected = set()
        ...
        for node in candidate_nodes:
            if is_directed:
                dd[node] = self.graph.in_degree(node)  # Use in-degree for directed graphs
            else:
                dd[node] = self.graph.degree(node)     # Use degree for undirected graphs
        ...
        while remaining_budget > 0 and dd:
            ...
            neighbors = list(self.graph.successors(max_node)) if is_directed else list(self.graph.neighbors(max_node))
            for neighbor in neighbors:
                if neighbor in dd:
                    t[neighbor] += 1
...
def save_split_ratio_results(results, split_ratio, base_filename="DegreeDiscount_Two_Phase_Results"):
...
def main():
    ...
    print("Running experiments with Degree Discount algorithm...")
